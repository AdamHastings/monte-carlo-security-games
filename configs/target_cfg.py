...
params_ranges = dict(
    ...
    MANDATE_range    = [0.45], # Percentage of assets that are spent on security measures.
    ATTACKERS_range  = [0.55], # Percentage of Attackers, relative to BLUE_PLAYERS
    INEQUALITY_range = [0.55], # How much poorer Attackers are than Defenders, as a pct
    ...
    SUCCESS_range    = [0.55], # Percentage of a defender's assets that must be spent by an Attacker to attempt an attack

    # Parameters that affect gameplay
    PAYOFF_range     = [1.0], # How much of a defender's assets are stolen if attacker is successful
    CAUGHT_range     = [0.5], # Probability that an attacker is caught and has their assets confiscated
