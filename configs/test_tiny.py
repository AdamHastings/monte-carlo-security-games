...
params_ranges = dict(
    ...
    MANDATE_range    = np.linspace(0.1, 0.8, 2), # Percentage of assets that are spent on security measures.
    ATTACKERS_range  = np.linspace(0.5, 1.0, 1), # Percentage of Attackers, relative to BLUE_PLAYERS
    INEQUALITY_range = np.linspace(0.3, 1.0, 1), # How much poorer Attackers are than Defenders, as a pct
    ...
    CAUGHT_range = np.linspace(0.1, 0.6, 2), # Probability that an attacker is caught and has their assets confiscated
