...
def main():
    try:
        ...
        print("\n     $ python3 run_cProfile.py test_small.py\n\n")
        sys.exit(0)
    ...
    for a in cfg.params_ranges["ATTACKERS_range"]:
        for b in cfg.params_ranges["PAYOFF_range"]:
            for c in cfg.params_ranges["INEQUALITY_range"]:
                for d in cfg.params_ranges["EFFICIENCY_range"]:
                    for e in cfg.params_ranges["EFFORT_range"]:
                        for f in cfg.params_ranges["CAUGHT_range"]:
                            for g in cfg.params_ranges["CLAIMS_range"]:
                                for h in cfg.params_ranges["PREMIUM_range"]:
                                    for i in cfg.params_ranges["TAX_range"]:
                                        for j in cfg.params_ranges["MANDATE_range"]:
                                            run_games(a,b,c,d,e,f,g,h,i,j)
