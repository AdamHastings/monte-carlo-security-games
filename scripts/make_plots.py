...
def crossover_hist(df):
    ...
    # plt.yscale("log")
    plt.title("CDF of crossover iterations (linear scale)")
    print(mandates)
    plt.legend(custom_lines, mandates)
...
def converge_hist(df):
    ...
    # plt.ylim(0,10000)
    plt.title("CDF of convergence iterations (linear scale)")
    # plt.yscale("log")
    plt.xlabel("Iteration")
    plt.ylabel("Count")
    plt.legend()
...
def loss_ratio_hist(df):
    ...
    plt.xlabel("Percent of assets lost")
