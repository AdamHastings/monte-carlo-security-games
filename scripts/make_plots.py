...
def crossover_hist(df):
    ...
    for i,m in enumerate(mandates):
        ...
        row = i//5
        col = i%5
        n, bins, patches = a[row, col].hist(crossovers, bins=100, cumulative=True)
        patches[0].set_fc('r')
        a[row, col].set_title("Mandate = " + str(m * 100) + "%")
    plt.yscale("log")
    plt.suptitle("Histogram of crossover iterations (log scale)")
...
def converge_hist(df):
    ...
    for i,m in enumerate(mandates):
        ...
        row = i//5
        col = i%5
        n, bins, patches = a[row, col].hist(convergences, bins=100,  cumulative=True)
        a[row, col].set_title("Mandate = " + str(m * 100) + "%")

    # plt.ylim(0,10000)
    plt.suptitle("Histogram of convergence iterations (log scale)")
...
def loss_ratio_hist(df):
    ...
    for i,m in enumerate(mandates):
        ...
        n, bins, patches = a[row, col].hist(ratios, bins=np.linspace(0,1,100),  cumulative=True)
        a[row, col].set_title("Mandate = " + str(m * 100) + "%")
    ...
    # plt.yscale("log")
    # plt.ylim(0,10000)
    plt.suptitle("Histogram of percent of assets lost (log scale)")
